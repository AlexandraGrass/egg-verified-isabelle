...
\setbeamercolor{palette primary}{bg=TUMBlue}
...
\begin{frame}{Proof format}
  \begin{center}
    ...
    $\Gamma \vdash p : x \approx y$\pause{} $\quad\:$ ``$p$ proves $x \approx y$ assuming $\Gamma$''
    \end{tcolorbox}
    \vspace{1.5em}
    \vfill
...
\begin{frame}{Refining $\ufds$}
  \begin{tikzpicture}[every node/.style={inner sep=2pt}]
    ...
    \begin{scope}[visible on=<+(1)->]
      \node[above right=0.2cm and 2cm of ufdsty1.east, draw, ellipse, TUMSecondaryBlue] (ufdsty1d) {union by size};
      \draw[<-, dashed] (ufdsty1) -- (ufdsty1d);
    \end{scope}
    ...
    \begin{scope}[visible on=<+(1)->]
      \node[above right=0.2cm and 2cm of ufdsty4.east, draw, ellipse, TUMSecondaryBlue] (ufdsty4d) {path compression};
      \draw[<-, dashed] (ufdsty4) -- (ufdsty4d);
    \end{scope}
  \end{tikzpicture}
  \vfill
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}[<+(1)->]
    \item Verified proof-producing union-find algorithm
    \item Generation of imperative code in e.g. StandardML
    \item Experiments show: $\sim$ factor 2 competitive with C\texttt{++} (factor 4 with big ints)
  \end{itemize}
\end{frame}
